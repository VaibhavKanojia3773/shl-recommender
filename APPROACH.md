...
- **Schema compliance.** A smoke-test script (`smoke_test.py`) exercises five representative conversation patterns — vague clarification, direct recommendation, off-topic refusal, comparison request, and explicit confirmation. Every response is validated against the Pydantic schema. The Groq `response_format={"type": "json_object"}` setting plus the four-stage parser have produced zero schema violations across hundreds of test calls.
- **Hallucination rate.** During development I ran the agent against several adversarial prompts ("Do you have the HireQuotient assessment?", "Add a Kotlin Backend Battery"). The model occasionally tried to oblige; the validator dropped every fabricated URL. Final hallucination rate in tested traces: **0 %**.
- **Behavioural alignment with sample conversations.** I read all ten C-traces before writing the prompt and used their patterns as a regression set. The agent successfully reproduces the layered batteries expected for senior Java hires (C7), declines legal interpretation while preserving the shortlist (C6), and ends the conversation on confirmation phrases without needing the turn-8 cap to fire.
...
- **Single-model agent.** Initially I had Llama 3.3 70B do both intent extraction and response generation. This was slightly cleaner but ~40 % slower per turn. Splitting keyword extraction to the 8B Instant model freed the larger model to focus on reasoning and the JSON contract.
